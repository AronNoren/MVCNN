...
class MultiviewImgDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root_dir, scale_aug=False, rot_aug=False, test_mode=False, \
        ...
        for i in range(len(self.classnames)):
            #print(os.getcwd().replace('\\','/')+'/'+parent_dir+'/'+self.classnames[i]+'/'+set_+'/*.jpg')
            all_files = sorted(glob.glob(os.getcwd().replace('\\','/')+'/'+parent_dir+'/'+self.classnames[i]+'/'+set_+'/*.jpg'))
            #print(all_files)
    ...
    def __getitem__(self, idx):
        ...
        class_name = path.split('/')[-2]#edited for windows
...
class SingleImgDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        class_name = path.split('/')[-2] #changedfrom -3 for windows
        print(class_name)
        print(path)
        class_id = self.classnames.index(class_name)
